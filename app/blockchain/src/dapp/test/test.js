...
describe("Contrato de Votação", function () {
    ...
    async function createElection(name, description, withPeriod = false) {
        ...
        if (withPeriod) {
            await voting["createElection(string,string,uint256,uint256,uint256[])"](
                name, description, currentTime + DURATION, currentTime + 2 * DURATION, [0, 1]
            );
        } else {
            await voting["createElection(string,string,uint256[])"](name, description, [0, 1]);
        }
    }
    ...
    describe("Admin", function () {
        ...
        describe("Gestão de District", function () {
            it("Deve adicionar um distrito", async function () {
                await createElection("Eleição A", "Descrição A");
                await voting.connect(admin).addDistrict(0, "District 1", "Distrito de Goiânia");
                const district = await voting.getElectionDistrict(0, 0);
                expect(district.name).to.equal("District 1");
                expect(district.description).to.equal("Distrito de Goiânia");
            });
        });
    });
    ...
    describe("Integração entre funções", function () {
        it("Deve adicionar e atualizar um eleitor", async function () {
            await createElection("Eleição A", "Descrição A");
            await voting.connect(admin).addVoter(0, "Voter 1", "password1", voter1.address);
            await voting.connect(admin).updateVoter(0, 0, "Voter 1 Updated", "password2", 2);
            const voter = await voting.getElectionVoter(0, 0);
        ...
        it("Deve adicionar um distrito e verificá-lo na eleição", async function () {
            await createElection("Eleição A", "Descrição A");
            await voting.connect(admin).addDistrict(0, "District 1", "Distrito de Goiânia");
            const district = await voting.getElectionDistrict(0, 0);
            expect(district.name).to.equal("District 1");
    ...
    describe("Segurança", function () {
        ...
        it("Deve permitir apenas o administrador adicionar um distrito", async function () {
            await createElection("Eleição A", "Descrição A");
            await expect(voting.connect(candidate1).addDistrict(0, "District 1", "Distrito de Goiânia")).to.be.revertedWith("Only Admins can perform this action");
        });
    });
    ...
    describe("Fluxo de Votação", function () {
        ...
        it("Deve retornar todas as eleições de um distrito", async function () {
            await createElection("Eleição A", "Descrição A");
            await createElection("Eleição B", "Descrição B");
            const elections = await voting.getDistrictElections(1);
            expect(elections.length).to.equal(2);
            const electionA = await voting.connect(admin).getElection(0)
            const electionB = await voting.connect(admin).getElection(1)
            expect(electionA.name).to.equal("Eleição A");
